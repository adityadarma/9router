...
export {
  ...
  validateApiKey, validateApiKeyDetailed, addTokensUsedByKey, touchApiKeyUsed, keyLimitReason, keyModelAllowed, checkApiKeyContextLimit,
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    for (const k of payload.apiKeys || []) {
      db.run(
        `INSERT OR REPLACE INTO apiKeys(id, key, name, machineId, isActive, createdAt, tokenLimit, expiresAt, tokensUsed, allowedModels, lastUsedAt) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [k.id, k.key, k.name || null, k.machineId || null, k.isActive === false ? 0 : 1, k.createdAt || new Date().toISOString(), k.tokenLimit != null ? Number(k.tokenLimit) : null, k.expiresAt || null, k.tokensUsed != null ? Number(k.tokensUsed) : 0, stringifyJson(Array.isArray(k.allowedModels) ? k.allowedModels : []), k.lastUsedAt || null]
      );
    }
